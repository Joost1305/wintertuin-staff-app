...
async function renderRosterWeek() {
  ...
  html += `<div class="cardRow" style="margin-bottom:14px; align-items:center;">
    ...
    <div>
      ...
      ${isManager() ? `<button class="btn secondary" onclick="downloadTeamMonthIcs()">Download hele team (.ics)</button>` : ""}
    </div>
  </div>`;
      ...
      <div style="display:flex; flex-wrap:wrap; gap:8px;">
        ${rosterTemplatesCache.map(t => `
          <div class="mono" draggable="true" ondragstart="dragTemplate(event, '${t.id}')"
            ...
            <span>${esc(t.name)}: ${fmtTime(t.start_time)}&ndash;${fmtTime(t.end_time)}</span>
...
function openAddShift(staffId, dateStr) {
  ...
  el("shiftFormHolder").innerHTML = `<div class="managerBox">
    ...
    <div class="field"><label>Van</label><input id="shiftStart" type="time" value="09:00"></div>
    <div class="field"><label>Tot</label><input id="shiftEnd" type="time" value="17:00"></div>
...
function editShiftInline(shiftId) {
  ...
  el("shiftFormHolder").innerHTML = `<div class="managerBox">
    ...
    <div class="field"><label>Van</label><input id="shiftStart" type="time" value="${fmtTime(s.start_time)}"></div>
    <div class="field"><label>Tot</label><input id="shiftEnd" type="time" value="${fmtTime(s.end_time)}"></div>
...
async function saveTemplate() {
  ...
  const { error } = await sb.from("shift_templates").insert({ name, start_time, end_time, section });
  if (error) { alert("Opslaan mislukt: " + error.message); return; }
  loadRoster();
}
